<?php
/*
Plugin Name: NFL Football Power Rankings Lite
...
function NFLSPRLPageURL() {
 $NFLSPRLpageURL = 'http';
 $NFLSPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NFLSPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NFLSPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NFLSPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nflsprlstats() 
{
?>
<h2>NFL Power Rankings</h2>
<?php nflsprl_stats(); ?>
<?php
}

function nflsprlstats_install()
{
register_sidebar_widget(__('NFL Power Rankings Lite'), 'widget_nflsprlstats'); 
}
add_action("plugins_loaded", "nflsprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nflsprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nflsprl_stats_remove' );

function nflsprl_stats_install() 
{
...
add_option("nflsprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nflsprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nflsprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NFL Power Rankings Installation", "Hi\n\nLITE NFL Power Rankings Activated at \n\n".NFLSPRLPageURL()."\n\nNFL Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nflsprl_stats_remove() 
{
/* Deletes the database field */
delete_option('nflsprl_scroll_text_color');
delete_option('nflsprl_scroll_text_color1');
delete_option('nflsprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nflsprl_stats_admin_menu');

function nflsprl_stats_admin_menu() {

add_options_page('NFL Power Rankings Lite', 'NFL Power Rankings Lite Settings', 'administrator', 'nfl-power-rankings-lite.php', 'nflsprl_stats_plugin_page');
}
}
function nflsprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nflsprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nflsprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nflsprl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nfl_power_rankings_magpie_lite/nfl_power_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nflsprl_fetch_rss( $url );
...
function nflsprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NFL Power Rankings Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NFL Power Rankings Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nfl-power-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nfl-power-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
